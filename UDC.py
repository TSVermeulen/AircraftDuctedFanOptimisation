...
choking handling, crash detection, and grid refinement in the form of the
Unified Ducted fan Code (UDC). It builds on the MTSET_call, MTFLO_call, and
MTSOL_call classes to construct the complete interface. The module is able to
maintain a logging file to allow for debugging through the use of an optional
control boolean.

Classes
-------
UDC : class
    A class to execute the complete MTFLOW interface. Effectively a detailed
    "wrapper" class of the MTSET_call, MTFLO_call, and MTSOL_call classes.
...
Changelog:
- V1.0: Initial version. Lacks proper crash handling and choking handling in
        the HandleExitFlag() method, but otherwise complete.
- V1.1: Cleaned up following successful implementation of validation case.
        Crash handling is now done within MTSOL_call.
- V1.2: Cleaned up import statements, resolved infite loop issue in gridtest,
        and added MTSOL loop exit for non-convergence.
- V1.3: Removed HandleExitFlag() method as it is not needed. Extracted choking
        handling to a separate method.
- V1.4: Cleaned up imports. Implemented chdir context manager. Switched to
        pathlib for path operations. Cleaned up/streamlined exit flags.
        Implemented OutputType enum class.
- V1.5: Updated to remove iter_count output from MTSOl_call.
        Updated documentation. Reduced no of streamwise points to 150 to
        reduce computational cost. Adopted named constants for grid
        coefficients and streamwise points.
...
class UDC:
    ...
    This class handles the setup, execution, and error handling for the MTFLOW
    evaluation process, which includes generating input files,
    constructing grids, and running solvers.
    """
    ...
    def caller(self,
        ...
        # context manager. Execute all code within the context manager
        # --------------------

        with change_working_directory(self.submodels_path):
            # --------------------
            # First step is generating the MTSET input file -
            # walls.analysis_name if needed
            # --------------------

            # Check if all required inputs are present in case the inputs
            # need to be constructed
            if not external_inputs:
                required_kwargs = ("centrebody_params","duct_params",
                                   "blading_parameters","design_parameters")
            ...
            # --------------------
            # Check the grid by running a simple, fan-less,
            # inviscid low-Mach case. If there is an issue with the grid MTSOL
            # will crash. Hence we can check the grid by checking the exit flag
            # --------------------
            ...
            while exit_flag != ExitFlag.SUCCESS:
                # If the grid is incorrect, change grid parameters and rerun
                # MTSET to update the grid. The updated e and x coefficients
                # reduce the number of streamwise points on the airfoil
                # elements (by 0.1 * Npoints), while yielding a more
                # "rounded/elliptic" grid due to the reduced x-coefficient.
                if check_count == 0:
                    # For the initial attempt, use the default values,
                    # but with an increased streamwise resolution
                    ...
                    # Revert back to the default number of streamwise points -
                    # this can help reduce likeliness of self-intersecting grid
                    streamwise_points = self.INITIAL_STREAMWISE_POINTS
                    grid_e_coeff = 0.7
                    grid_x_coeff = 0.9
                else:
                    # If the grid is still incorrect after the repair attempt,
                    # we assume that the grid is not fixable and exit the loop
                ...
                exit_flag, forces_data  = MTSOL_call(operating_conditions={"Inlet_Mach": 0.15,
                                                                           "Inlet_Reynolds": 0.,
                                                                           "N_crit": self.operating_conditions["N_crit"]},
            ...
            if exit_flag != ExitFlag.CRASH:
                try:
                    ...
                    MTFLO_call(self.analysis_name).caller()
                except Exception as e:
                    # If MTFLO fails, set the exit flag to crash so that we
                    # don't execute the MTSOL evaluation.
                    exit_flag = ExitFlag.CRASH
                    print(f"MTFLO failed: {e}")

                if exit_flag != ExitFlag.CRASH:
                    # Execute MTSOL
                    exit_flag, forces_data = MTSOL_call(operating_conditions=self.operating_conditions,
                                                        analysis_name=self.analysis_name).caller(run_viscous=self.run_visc,
                                                                                                 generate_output=True,
                                                                                                 output_type=output_type)
        return exit_flag, forces_data
...
if __name__ == "__main__":
    ...
    # Collect sea level atmosphere properties and calculate the mach number
    # and reynolds number at the inlet
